...
const averageWordLength = arr => {
  ...
  if (arr.length === 0) {
    ...
    sum = lengthArr.reduce(myFunc);
    return (avg = sum / lengthArr.length);
  }
      ...
      type = typeof arr[i];
        switch(type) {
          ...
          case 'string':
            ...
            break;
          default:
            console.log("Oops! boolean, buddy")
        }        
  }
let x = accum/arr.length;
...
const howManyTimes = (arr, word) => {
  ...
  if (arr.length === 0) {
   ...
   let str = arr.join(' ');
    ...
    return str.includes(word) ?  str.match(re).length : 0
...
const matrix = 
  ...
  function greatestProduct(matrix){
    ...
    for (var column = 0; column < matrix[row].length - 4; column++) {
    var horizontalProduct = 1;
    for (var col = column; col < (column + 4); col++) {
    horizontalProduct *= matrix[row][col];
    }
    ...
    verticalProduct *= matrix[r][column];
    }
